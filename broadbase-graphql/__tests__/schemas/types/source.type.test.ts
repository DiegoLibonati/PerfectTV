...
describe("source.type", () => {
  it("should have name SourceType", () => {
    expect((SourceType as GraphQLObjectType).name).toBe("SourceType");
  });

  it("should expose the expected field names", () => {
    const fields: GraphQLFieldMap<unknown, unknown> = (SourceType as GraphQLObjectType).getFields();

    expect(Object.keys(fields).sort()).toEqual(["base", "channels", "code", "description", "id"]);
  });
  ...
  it("should mark base as nullable BaseType", () => {
    const fields: GraphQLFieldMap<unknown, unknown> = (SourceType as GraphQLObjectType).getFields();

    expect(fields.base!.type.toString()).toBe("BaseType");
  });
});
